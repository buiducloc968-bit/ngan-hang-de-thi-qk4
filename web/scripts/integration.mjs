...
try{
 ...
 // --- Lop / don vi hoc vien ---
 const cls=await good('/classrooms',admin,'POST',{name:'Lớp kiểm thử A'});
 const cls2=await good('/classrooms',admin,'POST',{name:'Lớp kiểm thử B'});
 check(Boolean(cls.id)&&(await good('/classrooms',teacher)).some(c=>c.id===cls.id&&c.students===0),'Admin creates a classroom that teachers can see');
 check((await req('/classrooms',teacher,'POST',{name:'Không được phép'})).status===403,'Teacher cannot create a classroom');
 const studentId=(await good('/me',student)).id;
 await good('/users/'+studentId,admin,'PATCH',{classroomId:cls.id});
 check((await good('/students',teacher)).find(x=>x.id===studentId).classroomId===cls.id,'Admin puts a student into a classroom');
 check((await req('/classrooms/'+cls.id,admin,'DELETE')).status===400,'Classroom with students cannot be deleted');
 const classExam=await good('/exams',teacher,'POST',{...p,name:'Đề kiểm tra giao theo lớp',questionIds:[],assignedClassIds:[cls2.id]});
 await good('/exams/'+classExam.id+'/publish',teacher,'PATCH',{published:true});
 check(!(await good('/exams',student)).some(e=>e.id===classExam.id),'Exam assigned to another classroom stays hidden');
 check((await req('/exams/'+classExam.id+'/start',student,'POST')).status===403,'Student outside the assigned classroom cannot start');
 await good('/users/'+studentId,admin,'PATCH',{classroomId:cls2.id});
 check((await good('/exams',student)).some(e=>e.id===classExam.id),'Exam appears after the student joins the assigned classroom');
 check((await good('/attempts?classroomId='+cls2.id,teacher)).every(x=>x.classroom==='Lớp kiểm thử B'),'Attempt list filters by classroom');
 check((await good('/stats?classroomId='+cls.id,teacher)).attempts===0,'Statistics can be narrowed to one classroom');

 // --- Xuat ket qua: bang diem Excel va bien ban Word ---
 const sheetRes=await fetch(base+'/api/attempts?format=xlsx',{headers:{Authorization:'Bearer '+teacher}});
 const ExcelJS=(await import('exceljs')).default;const book=new ExcelJS.Workbook();await book.xlsx.load(Buffer.from(await sheetRes.arrayBuffer()));
 const sheet=book.getWorksheet('Bảng điểm'),summary=book.getWorksheet('Tổng hợp theo đề');
 check(sheetRes.headers.get('content-disposition').includes('bang-diem.xlsx')&&sheet.rowCount>1&&summary.rowCount>1,'Score sheet exports attempt rows plus per-exam summary');
 check(sheet.getRow(2).getCell(2).text.length>0&&['Đạt','Chưa đạt','Chờ chấm tự luận','—'].includes(sheet.getRow(2).getCell(11).text),'Score sheet carries student name and readable result');
 check(sheet.getRow(2).getCell(4).text==='Lớp kiểm thử B','Score sheet includes the classroom column');
 check((await good('/attempts?examId='+exam.id,teacher)).every(x=>x.examName===exam.name),'Attempt list can be filtered by exam');
 check((await req('/attempts/'+a.id+'/export',other)).status===403,'Student cannot download another student result record');
 const own=await fetch(base+'/api/attempts/'+a.id+'/export',{headers:{Authorization:'Bearer '+student}});
 const ownBytes=new Uint8Array(await own.arrayBuffer());check(own.ok&&ownBytes[0]===80&&ownBytes[1]===75&&ownBytes.length>2000,'Student downloads own result record as Word');
 // --- Cham bai thi giay theo ma de ---
 const variants=await good('/exams/'+exam.id+'/variants',teacher,'POST',{count:2});
 const firstKey=variants.keys[0];
 const keyString=firstKey.answers.map(x=>x==='—'?'-':x).join(' ');
 const paper=await good('/exams/'+exam.id+'/paper',teacher,'POST',{variant:firstKey.code,entries:[{userId:studentId,answers:keyString}]});
 check(paper.count===1&&paper.results[0].right===firstKey.answers.filter(x=>x!=='—').length&&paper.results[0].wrong===0,'Paper answers are graded against the variant answer key');
 check(paper.results[0].status===(paper.results[0].pending?'PENDING':'GRADED'),'Paper attempt waits for essay grading when the exam has one');
 check((await good('/attempts?examId='+exam.id,teacher)).some(x=>x.id===paper.results[0].id&&x.paper===true),'Paper attempt appears in the result list marked as paper');
 check((await req('/exams/'+exam.id+'/paper',student,'POST',{entries:[{userId:studentId,answers:'A'}]})).status===403,'Student cannot enter paper results');
 check((await req('/exams/'+exam.id+'/paper',teacher,'POST',{variant:firstKey.code,entries:[{userId:studentId,answers:'A Z'}]})).status===400,'Invalid answer letters are rejected');
 check((await req('/exams/'+exam.id+'/paper',teacher,'POST',{variant:'999',entries:[{userId:studentId,answers:'A'}]})).status===404,'Unknown variant code is rejected');
 check((await req('/exams/'+exam.id+'/paper',t2,'POST',{entries:[{userId:studentId,answers:'A'}]})).status===403,'Only the owning teacher can enter paper results');
