...
import {word,excel,resultsExcel,resultWord} from './exports.js';
...
// --- Chấm bài thi giấy: nhập chuỗi đáp án theo mã đề, phần mềm chấm và đưa vào thống kê như bài trực tuyến ---
const paperSchema=z.object({variant:z.string().trim().max(10).default(''),entries:z.array(z.object({userId:z.number().int(),answers:z.string().trim().max(2000)})).min(1).max(200)});
function parsePaperAnswers(text:string,qs:Q[]){
 const tokens=text.toUpperCase().split(/[\s,;.|]+/).filter(Boolean);
 if(tokens.length>qs.length)fail(`Đã nhập ${tokens.length} câu trả lời nhưng đề chỉ có ${qs.length} câu.`);
 const answers:Record<number,number[]>={};
 tokens.forEach((t,i)=>{
  if(qs[i].type==='SHORT'||t==='-'||t==='_')return; // câu tự luận và câu bỏ trống để giáo viên chấm sau
  if(!/^[A-F]+$/.test(t))fail(`Câu ${i+1}: "${t}" không hợp lệ. Dùng chữ cái A–F, nhiều đáp án viết liền (ví dụ AC), bỏ trống ghi dấu gạch.`);
  const idx=[...new Set(t.split('').map(c=>c.charCodeAt(0)-65))].sort((a,b)=>a-b);
  if(idx.some(j=>j>=qs[i].options.length))fail(`Câu ${i+1}: phương án vượt quá số lựa chọn của câu.`);
  answers[i]=idx;});
 return answers;}
app.post('/api/exams/:id/paper',run(async(r,s)=>{staff(r);
 const e=await db.exam.findUniqueOrThrow({where:{id:Number(r.params.id)}});owner(r,e.creatorId);
 const p=paperSchema.parse(r.body);
 const base:Q[]=JSON.parse(e.questions);
 const v=p.variant?(JSON.parse(e.variants) as Variant[]).find(x=>x.code===p.variant):null;
 if(p.variant&&!v)fail('Không tìm thấy mã đề này.',404);
 const qs=v?buildVariant(base,v):base;
 const ids=[...new Set(p.entries.map(x=>x.userId))];
 const students=await db.user.findMany({where:{id:{in:ids},role:'STUDENT',active:true},select:{id:true,name:true}});
 if(students.length!==ids.length)fail('Danh sách học viên không hợp lệ.');
 const now=new Date();const results=[];
 for(const entry of p.entries){
  const answers=parsePaperAnswers(entry.answers,qs);
  const m=mark(qs,answers);
  const last=await db.attempt.findFirst({where:{examId:e.id,userId:entry.userId},orderBy:{attemptNo:'desc'},select:{attemptNo:true}});
  const a=await db.attempt.create({data:{examId:e.id,userId:entry.userId,attemptNo:(last?.attemptNo||0)+1,variant:p.variant,paper:true,startedAt:now,expiresAt:now,submittedAt:now,snapshot:JSON.stringify(qs),answers:JSON.stringify(answers),score:m.score,status:m.pending?'PENDING':'GRADED'}});
  results.push({id:a.id,userId:entry.userId,name:students.find(u=>u.id===entry.userId)?.name,score:m.score,right:m.right,wrong:m.wrong,pending:m.pending,status:a.status});
 }
 await audit(r,'PAPER_GRADE','Exam',e.id,`${results.length} bài thi giấy${p.variant?' · mã đề '+p.variant:''}`);
 s.status(201).json({count:results.length,results});}));
...
app.get('/api/attempts',run(async(r,s)=>{const where:any={...attemptScope(r)};if(r.query.examId)where.examId=Number(r.query.examId);if(r.query.classroomId)where.user={classroomId:Number(r.query.classroomId)};const rows=await expireDue(await db.attempt.findMany({where,select:{id:true,status:true,score:true,startedAt:true,submittedAt:true,expiresAt:true,attemptNo:true,paper:true,exam:{select:{name:true,passScore:true}},user:{select:{name:true,username:true,classroom:{select:{name:true}}}}},orderBy:{id:'desc'}}));const list=rows.map(a=>({id:a.id,student:a.user.name,username:a.user.username,classroom:a.user.classroom?.name||'',examName:a.exam.name,attemptNo:a.attemptNo,paper:a.paper,status:a.status,score:a.score,startedAt:a.startedAt,submittedAt:a.submittedAt,passScore:a.exam.passScore}));if(r.query.format==='xlsx')return void s.type('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet').attachment('bang-diem.xlsx').send(await resultsExcel(list));s.json(list);}));
app.get('/api/attempts/:id',run(async(r,s)=>s.json(attemptView(await getAttempt(r)))));
// Biên bản kết quả một bài làm: học viên tải bài của mình, cán bộ/giáo viên tải bài thuộc đề của mình.
app.get('/api/attempts/:id/export',run(async(r,s)=>{const a=await getAttempt(r);if(a.status==='IN_PROGRESS')fail('Bài làm chưa nộp nên chưa xuất được kết quả.');s.type('application/vnd.openxmlformats-officedocument.wordprocessingml.document').attachment(`ket-qua-${a.id}.docx`).send(await resultWord({...attemptView(a),user:a.user}));}));
...
const [qs,rows,exams]=await Promise.all([student?[]:db.question.findMany({where:qScope(r),select:{topic:true,level:true,type:true,audience:true}}),db.attempt.findMany({where:{...attemptScope(r),...(r.query.classroomId?{user:{classroomId:Number(r.query.classroomId)}}:{})},select:{id:true,status:true,score:true,expiresAt:true,snapshot:true,answers:true,grades:true,exam:{select:{passScore:true}}}}),db.exam.count({where:student?{published:true}:scope(r)})]);
...
// --- Lớp / đơn vị học viên: giao đề và thống kê theo tập thể ---
const classroomSchema=z.object({name:z.string().trim().min(2).max(100),note:z.string().trim().max(300).default('')});
app.get('/api/classrooms',run(async(r,s)=>{staff(r);const rows=await db.classroom.findMany({orderBy:{name:'asc'},include:{_count:{select:{students:true}}}});s.json(rows.map(c=>({id:c.id,name:c.name,note:c.note,students:c._count.students})));}));
app.post('/api/classrooms',run(async(r,s)=>{if(r.user.role!=='ADMIN')fail('Chỉ quản trị viên được quản lý lớp / đơn vị.',403);const c=await db.classroom.create({data:classroomSchema.parse(r.body)});await audit(r,'CLASS_CREATE','Classroom',c.id,c.name);s.status(201).json(c);}));
app.patch('/api/classrooms/:id',run(async(r,s)=>{if(r.user.role!=='ADMIN')fail('Chỉ quản trị viên được quản lý lớp / đơn vị.',403);const c=await db.classroom.update({where:{id:Number(r.params.id)},data:classroomSchema.partial().parse(r.body)});await audit(r,'CLASS_UPDATE','Classroom',c.id,c.name);s.json(c);}));
app.delete('/api/classrooms/:id',run(async(r,s)=>{if(r.user.role!=='ADMIN')fail('Chỉ quản trị viên được quản lý lớp / đơn vị.',403);const id=Number(r.params.id);const c=await db.classroom.findUniqueOrThrow({where:{id},include:{_count:{select:{students:true}}}});if(c._count.students)fail(`Lớp còn ${c._count.students} học viên nên chưa xóa được.`);await db.classroom.delete({where:{id}});await audit(r,'CLASS_DELETE','Classroom',id,c.name);s.json({ok:true});}));
app.get('/api/students',run(async(r,s)=>{staff(r);s.json(await db.user.findMany({where:{role:'STUDENT',active:true},select:{id:true,name:true,username:true,classroomId:true,classroom:{select:{name:true}}},orderBy:{name:'asc'}}));}));
app.get('/api/users',run(async(r,s)=>{if(r.user.role!=='ADMIN')fail('Chỉ quản trị viên được truy cập.',403);s.json(await db.user.findMany({select:{id:true,name:true,username:true,role:true,active:true,department:true,classroomId:true,classroom:{select:{name:true}}}}));}));
...
app.patch('/api/users/:id',run(async(r,s)=>{if(r.user.role!=='ADMIN')fail('Không có quyền.',403);const id=Number(r.params.id);const p=z.object({active:z.boolean().optional(),password:z.string().min(6).max(100).optional(),department:z.string().trim().max(100).optional(),classroomId:z.number().int().nullable().optional()}).parse(r.body);
 if(p.classroomId!=null&&!await db.classroom.count({where:{id:p.classroomId}}))fail('Lớp / đơn vị không tồn tại.');if(id===r.user.id&&p.active===false)fail('Không thể khóa tài khoản đang đăng nhập.');await db.user.update({where:{id},data:{...p,...(p.password?{password:await bcrypt.hash(p.password,12),recoveryHash:null,tokenVersion:{increment:1}}:{})}});await audit(r,p.password?'USER_RESET_PASSWORD':p.active===false?'USER_LOCK':p.active===true?'USER_UNLOCK':'USER_UPDATE','User',id,p.department!==undefined?'khoa/tổ: '+(p.department||'(trống)'):'');s.json({ok:true});}));
